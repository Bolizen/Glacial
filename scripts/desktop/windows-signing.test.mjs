...
const RELEASE_ID = "Glacial-0.9.0-ffffffffffff-20260720T120000Z";
...
function sourceState(overrides = {}) {
  return {
    ...
    version: "0.9.0",
    status: "",
    versions: { packageJson: "0.9.0", tauri: "0.9.0" },
    ...overrides,
  };
}
...
test("command provider keeps the file as one direct argument and forwards only named environment", () => {
  ...
  const releaseEnvironment = signingEnvironment(source, "Glacial-0.9.0-ffffffffffff-20260719T120000Z");
...
test("Tauri signing evidence requires one transient uninstaller between plugins and final installer", () => {
  ...
  const installer = join(TEST_ROOT, "bundle", "Glacial_0.9.0_x64-setup.exe");
  mkdirSync(dirname(capture), { recursive: true });
  writeFileSync(capture, minimalPe());
  const config = { expectedThumbprint: THUMBPRINT, applicationTarget: target, applicationCapture: capture };
...
test("release source revalidation rejects every mutable provenance field", () => {
  ...
  for (const changed of [
    ...
    { status: " M file" },
    { version: "0.9.1" },
    { versions: { packageJson: "0.9.0", tauri: "0.9.1" } },
...
test("release package commands and established version sources identify 0.9.0", () => {
  ...
  assert.match(releaseTool, /expected 0\.9\.0/);
  assert.match(signingTool, /\^Glacial-0\\\.9\\\.0-/);
  assert.match(changelog, /"version": "0\.9\.0"/);
  assert.match(readme, /Glacial v0\.9\.0 is licensed/);
  assert.match(signingDocs, /Glacial v0\.9\.0 is intended/);
});
