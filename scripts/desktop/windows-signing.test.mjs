...
const RELEASE_ID = "Glacial-0.9.7-ffffffffffff-20260720T120000Z";
...
function sourceState(overrides = {}) {
  return {
    ...
    version: "0.9.7",
    status: "",
    versions: { packageJson: "0.9.7", tauri: "0.9.7" },
    ...overrides,
  };
}
...
test("command provider keeps the file as one direct argument and forwards only named environment", () => {
  ...
  const releaseEnvironment = signingEnvironment(source, "Glacial-0.9.7-ffffffffffff-20260719T120000Z");
...
test("Tauri signing evidence requires one transient uninstaller between plugins and final installer", () => {
  ...
  const installer = join(TEST_ROOT, "bundle", "Glacial_0.9.7_x64-setup.exe");
  mkdirSync(dirname(capture), { recursive: true });
  writeFileSync(capture, minimalPe());
  const config = { expectedThumbprint: THUMBPRINT, applicationTarget: target, applicationCapture: capture };
...
test("release source revalidation rejects every mutable provenance field", () => {
  ...
  for (const changed of [
    ...
    { status: " M file" },
    { version: "0.9.2" },
    { versions: { packageJson: "0.9.7", tauri: "0.9.4" } },
...
test("release package commands and established version sources identify 0.9.7", () => {
  ...
  assert.deepEqual(
    ...
    ["0.9.7", "0.9.7", "0.9.7", "0.9.7"],
  );
  assert.match(cargo, /^version = "0\.9\.7"$/m);
  assert.match(cargoLock, /\[\[package\]\]\r?\nname = "glacial"\r?\nversion = "0\.9\.7"/);
  assert.match(releaseTool, /expected 0\.9\.7/);
  assert.match(signingTool, /\^Glacial-0\\\.9\\\.7-/);
  assert.match(backendVersion, /^GLACIAL_VERSION = "0\.9\.7"$/m);
  assert.match(changelog, /"version": "0\.9\.7"/);
  assert.match(readme, /Glacial v0\.9\.7 is licensed/);
  assert.match(releaseNotes, /^# Glacial 0\.9\.7 /);
  assert.match(signingDocs, /Glacial v0\.9\.7 is intended/);
  assert.match(formatPolicy, /Glacial application version \| `0\.9\.7`/);
  assert.match(lifecyclePolicy, /Status: v1 policy baseline for Glacial 0\.9\.7\./);
  assert.match(readinessAudit, /Audited product version: `0\.9\.7`/);
  assert.equal(readinessSnapshot.audited_version, "0.9.7");
});
...
test("manifest and SHA256SUMS verification detects post-packaging mutation", () => {
  ...
  const artifact = join(artifacts, "Glacial_0.9.7_x64-setup.exe");
  ...
  writeFileSync(manifestPath, JSON.stringify({ artifacts: [{ filename: "Glacial_0.9.7_x64-setup.exe", path: "artifacts/Glacial_0.9.7_x64-setup.exe", bytes: 11, sha256: hash }] }));
  writeFileSync(sumsPath, `${hash}  Glacial_0.9.7_x64-setup.exe\n`);
  assert.equal(verifyPublishedHashes(root, manifestPath, sumsPath), true);
  writeFileSync(manifestPath, JSON.stringify({ artifacts: [{ filename: "Glacial_0.9.7_x64-setup.exe", path: "artifacts/Glacial_0.9.7_x64-setup.exe", bytes: 11, sha256: "f".repeat(63) }] }));
