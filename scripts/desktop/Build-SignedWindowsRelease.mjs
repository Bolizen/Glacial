...
export function verifyReleaseSource(gitPath) {
  ...
  for (const [name, version] of Object.entries(versions)) if (version !== "0.6.9") throw new Error(`${name} identifies version ${version ?? "unknown"}; expected 0.6.9.`);
  if (!readFileSync(join(REPOSITORY, "backend", "app", "changelog.py"), "utf8").includes('"version": "0.6.9"')) throw new Error("Backend release metadata does not identify 0.6.9.");
  return { root, branch, commit, originMain, version: "0.6.9", versions, status: "" };
}
