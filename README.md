...
The selected executable must be the exact bundled Windows runtime, 64-bit CPython 3.13.13. The workflow rejects virtual-environment interpreters and every other Python version or architecture, creates a fresh `.desktop-build/runtime-inventory-venv`, installs the 14 exact runtime pins from `backend/requirements.lock.txt` with pip isolated mode, `--no-cache-dir`, and `--no-deps`, verifies the complete installed graph and `pip check`, and runs the current documentation/version and third-party inventory validators against that explicit interpreter and site-packages path before removing the disposable environment. Pip is treated only as virtual-environment bootstrap tooling. PyInstaller and setuptools remain governed by the separate desktop build lock and are not installed into this runtime metadata environment.
