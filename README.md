...
Glacial 0.9.12 closes validated scan-coverage, malformed UTF-8, and Node VCS shorthand metadata gaps on top of the 0.9.11 installed privacy and failure-recovery baseline. It does not claim public signing, a release candidate, stable readiness, or Glacial 1.0 authorization. Project-derived and user-authored values are sanitized before persistence and again at reports, remediation outputs, generated instructions, diagnostics, and release records. Production application traffic is authenticated loopback only. The WebView2 installer bootstrapper and explicitly invoked development, build, signing, support, or release tools may use external networks. See the [privacy and network disclosure](docs/privacy-and-network.md).
...
Glacial v0.9.12 is licensed under the Functional Source License, Version 1.1, ALv2 Future License (`FSL-1.1-ALv2`). It is Fair Source and source-available, but it is not presently OSI open source.
...
The selected executable must be the exact bundled Windows runtime, 64-bit CPython 3.13.13. The workflow rejects virtual-environment interpreters and every other Python version or architecture, downloads the 14 exact runtime wheels from official PyPI without a pip cache, verifies their filenames and SHA-256 values against `docs/release/python-artifact-integrity.json`, installs only that verified wheelhouse into a disposable environment, verifies the complete installed graph and `pip check`, and runs the current documentation/version and third-party inventory validators before removing the environment, wheelhouse, and generated hash requirements. Pip is treated only as virtual-environment bootstrap tooling. PyInstaller and setuptools remain governed by the separately locked and hash-provenanced desktop build scope.

The full Windows clean-environment release gate operates only on committed state in a disposable detached Git worktree. It uses the explicit base interpreter only as bootstrap transport for hash-verified wheels; the executed build/runtime interpreters come from the exact Python.org CPython 3.13.13 x64 embeddable archive pinned by repository SHA-256. It provisions pnpm with the frozen lockfile and isolated store/cache/state directories, carries an isolated locked `CARGO_HOME` through the actual Tauri build, reconstructs both Python trees from the authenticated archive and wheelhouses, binds complete executed-input tree receipts before and after use, checks disposable and primary Git states, and removes all disposable state on success or ordinary failure:

```powershell
node scripts/release/validate-clean-environment.mjs --python C:\Path\To\Python313\python.exe
```
...
- Review can generate an **Agent Remediation Brief**, the human-readable preview used for explicit copy or standalone Markdown download. The backend deterministically reconstructs bounded scanner explanations for unresolved findings and preserves redaction and coverage limitations.
- From the same immutable preview snapshot, a user can explicitly download an **Agent Remediation Package**, a deterministic data-only ZIP handoff for a human-selected external coding agent. It contains `README.md`, `AGENT_TASK.md`, `findings.json`, `manifest.json`, and `CHECKSUMS.sha256`; it contains no project source files, executable content, scripts, or symlinks.
- The Brief and Package include at most the first 100 unresolved findings in canonical severity-first order while reporting exact unresolved, included, and omitted counts. This is the accepted v1.0 single-handoff boundary, not a scan, storage, review, or display limit. See the [format compatibility and retention policy](docs/release/agent-remediation-format-policy.md).
- Neither remediation feature edits code autonomously, launches an agent, executes project content, installs dependencies, or writes into the scanned project.
...
- The selected project has a compact read-only Activity view that merges registration and scan history with append-only Project Expectations, drift-adoption, finding-review-completion, and dependency-approval events. Activity is newest-first, paginated, bounded, and does not log ordinary UI interactions.
