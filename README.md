...
Glacial 0.9.11 adds [installed privacy-sink, startup-failure, and state-recovery acceptance](docs/release/g056-installed-privacy-failure-recovery.md) to the 0.9.10 runtime-identity and signer-readiness baseline. The acceptance uses an unsigned internal installer and disposable state; it does not claim physical power-loss durability, a completed Deep Security Scan, public signing, a release candidate, or stable readiness. Project-derived and user-authored values are sanitized before persistence and again at reports, remediation outputs, generated instructions, diagnostics, and release records. Production application traffic is authenticated loopback only. The WebView2 installer bootstrapper and explicitly invoked development, build, signing, support, or release tools may use external networks. See the [privacy and network disclosure](docs/privacy-and-network.md).
...
Glacial v0.9.11 is licensed under the Functional Source License, Version 1.1, ALv2 Future License (`FSL-1.1-ALv2`). It is Fair Source and source-available, but it is not presently OSI open source.
