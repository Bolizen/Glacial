...
The full Windows clean-environment release gate operates only on committed state in a disposable detached Git worktree. It requires the same explicit base interpreter, provisions pnpm with the frozen lockfile and isolated store/cache/state directories, provisions Cargo metadata in an isolated `CARGO_HOME`, provisions both Python environments only from SHA-256-verified disposable wheelhouses, runs the release-relevant validation and build surface, checks the disposable and primary Git states, and removes all gate state on success or ordinary failure:
