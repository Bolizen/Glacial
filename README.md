...
Glacial 0.9.10 adds visible compile-time runtime identity, authenticated owned-backend version agreement, exact direct production dependency declarations, and a standalone signer preflight to the G052 accessibility/window baseline and the installed lifecycle, privacy, security-reporting, support, and notice boundaries established by G048 through G051. Configured signer values are not presented as observed signature evidence; unsigned, unavailable, self-signed, and publicly trusted states remain distinct. Project-derived and user-authored values are sanitized before persistence and again at reports, remediation outputs, generated instructions, diagnostics, and release records. Production application traffic is authenticated loopback only. The WebView2 installer bootstrapper and explicitly invoked development, build, signing, support, or release tools may use external networks. See the [privacy and network disclosure](docs/privacy-and-network.md).
...
Glacial v0.9.10 is licensed under the Functional Source License, Version 1.1, ALv2 Future License (`FSL-1.1-ALv2`). It is Fair Source and source-available, but it is not presently OSI open source.
