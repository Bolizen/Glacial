...
Glacial v0.7.2 is intended to use an Authenticode certificate with the subject `CN=Icefields Development`. The initial certificate is self-signed and is not publicly trusted. Its signature proves byte integrity and publisher-key continuity only; it does not establish Windows reputation or public trust.
...
1. Verify repository identity, branch, clean status, `HEAD == origin/main`, and v0.7.2 metadata.
