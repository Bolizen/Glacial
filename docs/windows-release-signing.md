...
Only the controlled-host administrator may rotate the anchor; the ordinary non-elevated release account must retain read-only access. Rotation requires an independently reviewed authorization record for the new offline authority, out-of-band verification of the new public-key digest, a repository review recording the authority transition, and a maintenance window in which release execution is stopped. The administrator atomically replaces the fixed-path public-key file, restores and verifies its owner and ACL, confirms the release account cannot modify the file or parent directory, and retires the previous anchor according to the offline key policy. The new offline authority must then issue a fresh schema-v2 manifest for the exact source, tools, signer, profile, and validity window. Installing the reviewed key at the protected fixed path is what authorizes the new authority on that host; changing invocation variables, detached manifest fields, sibling certificates, or repository worktree files cannot perform a rotation.
